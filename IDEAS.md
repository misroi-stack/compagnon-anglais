...
- [x] **4. Monter au niveau de la phrase** — ~700 phrases d'exemple en base, utilisées seulement en affichage (Flashcards) et en texte à trou (Phrase). Rien ne teste la **compréhension orale d'une phrase** ni la **lecture à voix haute d'une phrase**. Marche suivante évidente pour un 9 ans, contenu déjà là. *Livré le 2026-08-07 — intégré plutôt qu'en mode séparé (voir STATUS.md) : 3ᵉ type de question dans Quiz (écoute une phrase, choisis la bonne traduction) et variante "lis la phrase" dans Répète, tous deux réservés au niveau 3 de chaque thème (pas de notion d'âge dans le modèle de données).*
...
- [ ] **5. Une modalité écrite** — rien ne fait produire l'orthographe. L'anglais écrit est un vrai obstacle pour un francophone (*through, enough, colour*). Lettres mélangées à remettre en ordre pour le petit, saisie libre pour l'aîné : comblerait le seul trou dans les quatre modalités (écouter / parler / lire / **écrire**).
  - ⏸️ **Mis en pause le 2026-08-07** — jugé trop avancé pour l'instant par l'utilisateur, à reprendre plus tard. Recommandation discutée à ce moment-là, pour référence future : contrairement au point 4 (phrase), ce serait un **nouveau mode dédié** plutôt qu'intégré à un mode existant — écrire (taper ou remettre des lettres) est une interaction qui n'existe nulle part dans l'app (pas de champ de saisie ni de puzzle de lettres à réutiliser), donc forcer ça dans Quiz/Flashcards serait artificiel. Un seul mode "Écris" gérerait les deux formats selon le niveau du mot (même logique de palier que la phrase) : lettres mélangées aux niveaux 1-2, saisie libre au niveau 3. Compromis à garder en tête : contrairement au point 4 qui n'a ajouté aucun bouton, celui-ci en ajoute un — et pour **tous les thèmes** (pas juste Bouger comme TPR), donc le hub grimpe à 5-6 boutons partout.

- [x] **6. Distracteurs plus intelligents** — le dashboard parent *sait* quels mots l'enfant confond ; le Quiz l'ignore et tire ses distracteurs au hasard dans le même thème/niveau. Proposer *tree* comme distracteur de *three* **quand l'enfant a déjà confondu les deux** transforme un QCM générique en travail ciblé.
  - ✅ Corrigé le 2026-08-07 (en construisant la Session du jour) : `src/lib/quiz.ts` avait `const pool = allWords.length > 4 ? allWords : allWords;` — les deux branches identiques, une intention perdue. Remplacé par un vrai `distractorPool` optionnel.
  - ✅ Ciblage réel livré le 2026-08-07 — voir STATUS.md pour le détail (migration `008_add_selected_answer.sql`, `src/lib/confusion.ts`). Le dashboard parent, en réalité, ne connaissait QUE le taux de réussite par mot, pas les paires de confusion précises (ex. *three* confondu spécifiquement avec *tree*) — cette donnée n'existait nulle part avant cette livraison.
