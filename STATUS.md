...
- **Routing complet** : sélection profil → `/play/[profileId]` (grille des 24 thèmes) → `/play/[profileId]/[themeId]` (hub du thème, choix du mode) → `/play/[profileId]/[themeId]/[mode]` (l'écran de jeu)
- **Hub par thème** (`/play/[profileId]/[themeId]/page.tsx`) : après avoir choisi un thème, l'enfant reste dessus — les 4 modes s'affichent avec un compteur "X/Y réussis" (Quiz/Associe/Répète, basé sur `successModes` déjà suivi par mot) et une coche ✓ une fois complet. Terminer un jeu ramène à ce hub (même thème), pas à la grille complète — corrige un retour terrain où finir un jeu faisait dériver vers un thème différent au lieu d'encourager à faire les 4 jeux du même thème d'affilée. "Changer de thème" reste un lien explicite pour revenir à la grille.
